...
gas-report:
	...
	if [ -n "$${METRIC_RAW_OUTPUT_DIR:-}" ]; then \
		mkdir -p "$${METRIC_RAW_OUTPUT_DIR}/$$FULL_COMMIT"; \
		cp -R "$$GNO_WORKTREE/examples/gno.land/r/gnoswap/scenario/metric/filetests/." "$${METRIC_RAW_OUTPUT_DIR}/$$FULL_COMMIT/"; \
	fi; \
