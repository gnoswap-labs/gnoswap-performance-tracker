...
3. Run the requested benchmark using the printed branch or final commit:

   ```bash
   GNO_REF=gas-<8-char-upstream-sha> make metric <gnoswap-ref>
   GNO_REF=gas-<8-char-upstream-sha> make stress <gnoswap-ref>
   ```
